...
class TestRegion(TransactionCase):
    ...
    def test_01_create_region(self):
        ...
        region_data = {"name": "Another Test Region", "code": "AN", "int_code": "002"}
        ...
        self.assertEqual(region.int_code, "002", "Region int_code is incorrect")
    ...
    def test_03_create_region_empty_name(self):
        with self.assertRaises(ValidationError):
            self.env["g2p.region"].create({"name": "", "code": "A", "int_code": "TA"})

    def test_04_create_region_empty_code(self):
        with self.assertRaises(ValidationError):
            self.env["g2p.region"].create({"name": "A", "code": "", "int_code": "TA"})

    def test_05_create_region_empty_int_code(self):
        with self.assertRaises(ValidationError):
            self.env["g2p.region"].create({"name": "A", "code": "J", "int_code": ""})

    def test_06_create_region_duplicate_code(self):
        self.env["g2p.region"].create({"name": "Test", "code": "TCU", "int_code": "A"})
        with self.assertRaises(ValidationError):
            self.env["g2p.region"].create({"name": "Test", "code": "TCU", "int_code": "A"})
